...
<section class="max-w-6xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div>
        ...
        <form id="pos-form" method="post" action="pos-checkout.php" class="bg-white border border-gray-200 rounded p-4">
            <input type="hidden" name="cart_json" id="cart-json-input">
            <input type="text" name="customer_name" placeholder="Customer name (optional)"
                   class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-4">

            <div id="cart-items" class="space-y-2 mb-4 text-sm max-h-80 overflow-y-auto">
...
<script>
(function () {
    const cart = new Map(); // productId -> { name, price, stock, qty }
    ...
    document.querySelectorAll('.pos-product-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (existing) {
                ...
                cart.set(id, { name: btn.dataset.name, price: Number(btn.dataset.price), stock, qty: 1 });
            }
            render();
        });
    });
